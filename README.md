...
2. Contact the EWP Administration that you want to host a secondary ewp-registry-catalogue server. Send them public IPv4 of the machine that will be used as a ewp-registry-catalogue server.
3. Once your request is aproved you will receive:
...
4. Install `docker` on your server. You can find installation guide for you OS [here][docker-overview-supported-platforms] (Beware that `docker` package available in debian/ubuntu repository is NOT the package you want to install!)
5. Create GitHub access token. It will be used to download docker images released as packages and the catalogue. It should have `repo` and `read:packages` scopes. The token can be created [here][generate-github-access-token].
6. Login to GitHub's docker repository using your username and token generated in step 5:
```
...
10. Create application.properties file with the following contents, will GitHub username and token with those created in previous steps.
```properties
...
```
11. Run the following comand to run ewp-registry-catalogue in background, replace names in `<brackets>` with correct values:
```bash
docker run -dt \
        -p 80:80 \
        -p 443:443 \
...
```
example of command with filled placeholders:
```bash
docker run -dt \
        -p 80:80 \
        -p 443:443 \
...
15. Verify if you catalogue and DNS server is working from different machine.  
Repeat steps 13 and 14, but instead of `localhost` use `<Public IPv4 address of this server>`. It is important to use different machine in different network, as it will test if your server is available from the Internet.

16. After verifying if your server is available from the Internet inform the EWP Administration that you are ready.
...
```
indicates that you need to run those commands as a root (use sudo) or add your current user to `docker` group.

2. If running the command in step 11 returns a message simmilar to the following:
```
...
In case of debian/ubuntu OSes port 53 is probably used by systemd-resolved. You can stop this service using `sudo systemctl stop systemd-resolved`

3. `docker ps` lists the container as restarting.  
The container cannot start correctly if you haven't configured it properly.  
Logs from the container should contain a message of that is going on.  
...
This means that certificate used by `ewp-web.usos.edu.pl` (which is the main registry node) is not compatible with TLS private key you have received. Please check once again if you copied the right file, and if you are sure, then contact the EWP Administration.


# Running as a primary node.
If you are setting up a primary node, or setting up a node that will serve as a backup for ewp-registry-service, you should follow this scenario.
...
```
2. Prepare a directory for registry's data, here we will use `/var/ewp-registry-service` directory, but you can name it as you like.
```bash
mkdir /var/ewp-registry-service
```
3. Generate new RSA key pair. It will be used to access GitHub repository. You can follow [this guide][github-ssh-key-guide], but generate them into `/var/ewp-registry-service/.ssh` directory.
4. Send PUBLIC key (`/var/ewp-registry-service/.ssh/id_rsa.pub`) generated in step 3 to the EWP Administration. It will be used to push changes into the catalogue repository.
...
```
You will be asked to confirm RSA key fingerprint, verify if it matches any of fingerprints avaialbe [here][github-ssh-keys-fingerprints] and confirm.
...
docker run -it \
    ...
    docker.pkg.github.com/erasmus-without-paper/ewp-registry-service/ewp-registry-service
```
The Registry Service will be started and it's logs will be printed to your terminal. You shouldn't see any errors.

3. Check if the Registry is available locally. Visit `http://localhost:8080` in your browser and verify if main site of the Registry is visible. You can also use `curl http://localhost:8080` if you don't have a browser available.
4. Stop the Registry, press Ctrl+C in terminal from step 2.
...
You should run and stop your copy only on request of the EWP Administration.
...
docker pull docker.pkg.github.com/erasmus-without-paper/ewp-registry-service/ewp-registry-service:latest
```
2. Update the `manifest-sources.xml` file to the latest version. TODO
3. Run the registry.
```bash
...
5. If the EWP Administration asks you to, restart your copy of ewp-registry-catalogue as described in `Running ewp-registry-catalogue on a machine with EWP Registry running` section.
...
Preform steps described in `Running ewp-registry-catalogue on your machine.` section, with slightly different `docker run` command:
...
```
Note that you should use port different than 8080 in `--primary-service-uri` parameter if your registry is available on different port.  
...
Validate if your ewp-registry-catalogue service have started properly, as described in aforementioned section.  
